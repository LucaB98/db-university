...
SELECT S.`name` AS 'nome_studente', S.`surname` AS 'cognome_studente', C.`name` AS 'nome_corso', COUNT(ES.`vote`) AS 'numero Tentativi', MAX(ES.`vote`) AS `voto_massimo` FROM `students`AS S JOIN `exam_student`AS ES ON S.`id`= ES.`Student_id` JOIN `exams` AS E ON E.`id`= ES.`exam_id` JOIN `courses`AS C ON C.`id` = E.`course_id` GROUP BY S.`id`, C.`id` HAVING `voto_massimo` >= 18;